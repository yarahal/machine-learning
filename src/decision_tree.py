...
class ClassificationDecisionTree:
    ...
    def _get_proportions(self,y):
        p = np.zeros(len(self.classes))
        for i in range(len(p)):
            p[i] = np.mean(y==self.classes[i])
        return p

    def _criterion(self,j,s,X,y):
        p1 = self._get_proportions(y[X[:,j]<s])
        p2 = self._get_proportions(y[X[:,j]>=s])
    ...
    def _split(self,X_js,y_js,d,current_node):
        ...
        if(self.max_depth is None or d < self.max_depth-1):
            if(len(y_js[X_js[:,j_best]<s_best])>=self.min_samples_split):
                p1 = self._get_proportions(y_js[X_js[:,j_best]<s_best])
                current_node.left = TreeNode(np.argmax(p1))
                self._split(X_js[X_js[:,j_best]<s_best],y_js[X_js[:,j_best]<s_best],d+1,current_node.left)
            if(len(y_js[X_js[:,j_best]>=s_best])>=self.min_samples_split):
                p2 = self._get_proportions(y_js[X_js[:,j_best]>=s_best])
                current_node.right = TreeNode(np.argmax(p2))
                self._split(X_js[X_js[:,j_best]>=s_best],y_js[X_js[:,j_best]>=s_best],d+1,current_node.right)

    def fit(self,X,y):
        self.classes = np.unique(y)
        p = self._get_proportions(y)
        self.tree = TreeNode(np.argmax(p))
        self._split(X,y,0,self.tree)
